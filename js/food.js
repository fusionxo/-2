...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleNutritionSearch = async () => {
        ...
        try {
            const prompt = `Analyze the exact nutritional content for "${foodItemQuery}" per 100g. Provide a valid JSON object with ONLY these keys: "calories", "protein", "carbohydrates", "fat". The values must be numbers. Do not include any text, just the JSON.`;
            ...
            renderNutritionResults(foodItemQuery, baseNutritionData, quantityBeforeSearch);
     ...
     const renderNutritionResults = (foodItem, baseData, displayQuantity) => {
        ...
        dom.resultsContainer.innerHTML = `
            <div class="card p-4">
                <h4 class="font-bold text-lg capitalize">${foodItem}</h4>
        ...
        document.getElementById('log-from-lookup-btn').addEventListener('click', () => {
            if (window.healthHub && typeof window.healthHub.logFoodItem === 'function') {
                ...
                const foodItemWithQuantity = `${foodItem} (${finalQuantity}g)`;

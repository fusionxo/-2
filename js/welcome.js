/**
 * @fileoverview Handles the user onboarding/welcome flow.
...
const { auth, db, onAuthStateChanged, doc, updateDoc, serverTimestamp } = window.firebaseInstances;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cardData.push({
        ...
        id: 'step-2',
        validate: () => formContainer.querySelector('input[name="gender"]:checked')
    }, {
        id: 'step-3',
        validate: () => true
    }, {
        ...
        id: 'step-6',
        validate: () => formContainer.querySelector('input[name="activity"]:checked')
    }, {
        id: 'step-7',
        validate: () => true
    });
    ...
    function goToNextStep() {
        ...
        if (currentStep >= 0 && !cardData[currentStep].validate()) {
            cards[currentStep].animate(
                [{ transform: 'translateX(0)' }, { transform: 'translateX(-10px)' }, { transform: 'translateX(10px)' }, { transform: 'translateX(0)' }], { duration: 500, easing: 'ease-in-out' }
            );
            return;
        }
